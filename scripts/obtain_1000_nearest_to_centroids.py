...
if test_number == 0:
    ...
    neighbors_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'results','Balltree' ,'nearest_1000_neighbors_indices_0.npy'))
    ...
    #save the indices of the nearest neighbors for all centroids
    neighbors_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'results','Balltree' ,'nearest_1000_neighbors_indices_2.npy'))
    with open(neighbors_path, "wb") as f:
        numpy.save(f, ind)
    print(f"[{datetime.now()}] Nearest neighbors indices saved to {neighbors_path}.")
else:
