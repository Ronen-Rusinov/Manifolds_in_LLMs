...
def load_pca_embeddings(centroid_index: int, n_components: int) -> np.ndarray:
    """Load PCA embeddings for a specific centroid.
    ...
    Raises:
        FileNotFoundError: If embeddings file does not exist
    """
    embeddings_path = paths.get_pca_embeddings_path(centroid_index, n_components)
    if not embeddings_path.exists():
        raise FileNotFoundError(f"PCA embeddings file not found at {embeddings_path}")
    
    print(
        f"[{datetime.now()}] Loading PCA embeddings for centroid {centroid_index} from {embeddings_path}...",
        flush=True,
    )
    embeddings = np.load(embeddings_path)
    print(f"[{datetime.now()}] PCA embeddings loaded. Shape: {embeddings.shape}", flush=True)
    return embeddings


def batch_load_pca_embeddings(n_centroids: int, n_components: int) -> dict:
    """Load PCA embeddings for all centroids.
    ...
    Raises:
        FileNotFoundError: If any embeddings file does not exist
    """
    print(f"[{datetime.now()}] Preloading all {n_centroids} PCA embeddings into memory...", flush=True)
    all_embeddings = {}
    
    for i in range(n_centroids):
        try:
            embeddings_path = paths.get_pca_embeddings_path(i, n_components)
            all_embeddings[i] = np.load(embeddings_path)
            if (i + 1) % 50 == 0:
                print(f"[{datetime.now()}] Loaded {i + 1}/{n_centroids} PCA embeddings", flush=True)
        except FileNotFoundError:
            raise FileNotFoundError(
                f"Missing PCA embeddings for centroid {i} at {embeddings_path}. "
                f"Run pca_for_each_centroid script for all centroids first."
            )
    
    print(f"[{datetime.now()}] All PCA embeddings preloaded successfully", flush=True)
    return all_embeddings
