...
def compute_jacobian_orthogonality_loss(model, z, device='cpu'):
    ...
    """
    IN THE CASE OF A RELU ACTIVATION, WE CAN LEVERAGE THE FACT THAT
    THE DECODER BEHAVES LOCALLY LINEARELY,
    AND THE JACOBIAN OF LINEAR TRANSFORM IS ITSELF.
    """
    # While the _correct_ thing would be to compute it at every sample, we will instead go with a little bit
    # Lighter of a requirement,
    #That being ignoring activations entirely, and just
    ...
    # Compute J^T @ J for the decoder weight matrices
    #A TODO to actually implement it correctly at some point, but for now this is good enough
...
if __name__ == "__main__":
    ...
    val_data = torch.randn(batch_size, input_dim, device=device)
